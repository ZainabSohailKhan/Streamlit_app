...
# Display app title & introductory text
...
if uploaded_files:
    for file in uploaded_files:
        # Extract file extension --> determining if it's CSV or Excel
        file_extension = os.path.splitext(file.name)[-1].lower()
        
        # Read uploaded file into pandas DataFrame based on its extension
        if file_extension == ".csv":
            ...
            st.error(f"Unsupported file type: {file_extension}") # Showing error message if unsupported file type 
            continue
        
        st.write(f"**📄 File Name:** {file.name}")
        st.write(f"**📏 File Size:** {file.size / 1024:.2f} KB")  # File size: KB

        # Previewing first 5 rows of uploaded file
        st.write("🔍 Preview of the Uploaded File:")
        st.dataframe(df.head())  # Display scrollable data preview
        
        # Section: data cleaning options
        st.subheader("🛠️ Data Cleaning Options")
        if st.checkbox(f"Clean Data for {file.name}"):
            col1, col2 = st.columns(2)  # Split cleaning options into two columns
            with col1:
                # Button: removeing duplicate rows from DataFrame
                if st.button(f"Remove Duplicates from {file.name}"):
                    df.drop_duplicates(inplace=True)
                    st.write("Duplicates Removed!")
            with col2:
                # Button: fill missing numeric values with column means
                if st.button(f"Fill Missing Values for {file.name}"):
                    numeric_cols = df.select_dtypes(include=['number']).columns
                    df[numeric_cols] = df[numeric_cols].fillna(df[numeric_cols].mean())
                    st.write("Missing Values in Numeric Columns Filled with Column Means!")

        # Section: choose specific columns to convert
        st.subheader("🎯 Select Columns to Convert")
        columns = st.multiselect(f"Choose Columns for {file.name}", df.columns, default=df.columns)
        df = df[columns]  # Filters the DataFrame to the selected columns
        
        # Section: Visualization uploaded data
        st.subheader("📊 Data Visualization")
        if st.checkbox(f"Show Visualization for {file.name}"):
            st.bar_chart(df.select_dtypes(include='number').iloc[:, :2])  # Plot the first two numeric columns as a bar chart
        
        # Section: choose file conversion type (CSV or Excel)
...
if uploaded_files:  # Check if files were uploaded
    st.write("✅ Processing Completed!")  # Display a generic message
